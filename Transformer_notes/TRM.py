...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        # position embeding 矩阵 [max_len*d_model]
...
class EncoderLayer(nn.Module):
    def __init__(self):
        ...
        self.pos_ffn = PoswiseFeedForwardNet() # linear

    def forward(self, enc_inputs, enc_self_attn_mask):
        ## 下面这个就是做自注意力层，输入是enc_inputs，形状是[batch_size x seq_len_q x d_model]
        # 需要注意的是最初始的 QKV矩阵 是等同于这个输入的，去看一下enc_self_attn函数 6.
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        ## 下面这个代码通过src_emb，进行索引定位，转化为向量， enc_outputs输出形状是[batch_size, src_len, d_model]
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
        ...
        ## get_attn_pad_mask 自注意力层的时候的 pad 部分
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ## 这里有两个数据进行输入
        # 一个是enc_inputs 形状为[batch_size, src_len]，主要是作为编码段的输入
        # 一个dec_inputs，形状为[batch_size, tgt_len]，主要是作为解码端的输入

        # enc_outputs就是主要的输出，用于与 decoder 进行交互的
        # enc_self_attns 是 QK转置相乘之后softmax之后的矩阵值，代表的是每个单词和其他单词相关性；
...
if __name__ == '__main__':
    ...
    '''
    句子的输入部分解读
    'ich mochte ein bier P': ecoder 输入（德语）
    'S i want a beer': decoder 输入 （英语）
    'i want a beer E': decoder 的输出标签
    
    S: 表示 句子 start
    E: 表示 句子 end
    P: 表示 句子 空位，padding 到等长
    '''
