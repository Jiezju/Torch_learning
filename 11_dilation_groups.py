...
def matrix_mul_for_conv2d(x,
    ...
    kernel = kernel.reshape(groups, o_channel // groups, in_channel // groups,
                            kernel_h, kernel_w)
                ...
                for ic in range(in_channel // groups):
                    for i in range(0, in_h - kernel_h + 1, stride):
                        for j in range(0, in_w - kernel_w + 1, stride):
                            region = x[b, g, ic, i:i + kernel_h:dilation,
                                       j:j + kernel_w:dilation]
                            o[b, g, oc, i // stride, j // stride] += torch.sum(
