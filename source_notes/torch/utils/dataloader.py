...
class DataLoader(Generic[T_co]):
    ...
    num_workers: int
    pin_memory: bool # 将数据保存到 gpu 中
    drop_last: bool
    timeout: float
    sampler: Sampler # 自定义 选择样本的方式，而不是 纯粹 的 shuffle
    prefetch_factor: int
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
        ...
        if isinstance(dataset, IterableDataset):
            ...
            self._dataset_kind = _DatasetKind.Map # 默认 map 类型
        ...
        if batch_size is not None and batch_sampler is None:
            # auto_collation without custom batch_sampler 将 sampler 的样本 拼凑成 一个 batch
        ...
        if collate_fn is None:
            if self._auto_collation:
                # default_collate 只进行了 类型转换: batch 输入，处理 再以 batch 输出
                collate_fn = _utils.collate.default_collate
            else:
                collate_fn = _utils.collate.default_convert

        self.collate_fn = collate_fn  # 读入批量数据后，进行数据处理的函数
    ...
    # 实现 __iter__ 函数，允许使用 iter() 将该类型对象变为迭代器
